...
namespace ShowlightEditor.WPF.Controls
{
    ...
    public partial class NumericUpDown : UserControl
    {
        ...
        private static void OnValueChanged(DependencyObject d, DependencyPropertyChangedEventArgs args)
        {
            ...
            if(string.IsNullOrEmpty(nud.StringFormat))
                nud.txtNum.Text = newValue.ToString(FormatCulture);
            else
                nud.txtNum.Text = newValue.ToString(nud.StringFormat, FormatCulture);
        ...
        private void NumTextBox_PreviewTextInput(object sender, TextCompositionEventArgs e)
        {
            ...
            if (Minimum < 0)
                pattern += "-";
            if (DecimalPlaces > 0)
                pattern += FormatCulture.NumberFormat.NumberDecimalSeparator;
            pattern += "]";
